...
| `tte <watts>` | *How long could I hold this?* Time to exhaustion at a power you name, from the same Critical Power model `power-curve` fits. Every answer carries what qualifies it: which band of the model it falls in, and the longest effort at or above that power you already have on record — when the model predicts less than your own file proves, it says so. |
