...
| `progress <date>` | *Am I improving on this workout?* Resolves that day's workout(s) and shows every comparable instance chronologically, with **Efficiency Factor** (normalized power ÷ avg HR — watts per heartbeat) and a trend verdict. Climbing EF = fitter. Named classes match exactly; auto-named rides ("Morning Ride" = different routes) compare only rides within ±10% of the anchor's distance. |
