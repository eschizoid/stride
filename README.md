...
| `progress <name\|date>` | *Am I improving on this workout?* Every instance of a repeated session, chronological, with **Efficiency Factor** (normalized power ÷ avg HR — watts per heartbeat) and a trend verdict. Climbing EF = fitter. Pass a class name, or a **date** to resolve that day's workout automatically (`progress 2026-07-24`). Named classes only; routes aren't matchable (no GPS stored — that's Strava's segments). |
