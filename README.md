...
| `progress <name>` | *Am I improving on this workout?* Every instance of a repeated session (matched by exact name), chronological, with **Efficiency Factor** (normalized power ÷ avg HR — watts per heartbeat) and a trend verdict. Climbing EF = fitter. Works for named classes; routes aren't matchable (no GPS stored — that's Strava's segments). |
