...
- Junk HR (outside 35–220 bpm) is filtered at analyze time for stream SAMPLES, so sessions with bad straps
  ...
  is raw (unfiltered).
- The same 35–220 bound is applied again at REPORT time by four consumers, with three different consequences. `progress` counts the refused session in `hidden_lens` and omits it from `sessions[]`. `activity` publishes `ef.known: false` with `ef.current: 0`. The load ladder falls to its next rung and names it in `load_model`, so this one DOES change TSS — it is the consumer that propagates, into CTL, ATL and every form verdict. `top hr` does not rank the reading at all: it is absent from that list, its id is not recoverable from the JSON, and if every reading is out of band the list comes back EMPTY — `{"data":[]}` reads as "never recorded a heart rate" for an athlete who has 672 of them. To find the broken sessions themselves use `stride activities --json` and look for an `avg_hr` under 35 or over 220; that is the one surface publishing them raw.
