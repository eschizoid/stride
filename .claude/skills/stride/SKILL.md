...
| `stride season --json` | the season at block and month scale: `blocks` + `months` + `gap_weeks`. A BLOCK is a maximal run of consecutive training weeks closed by `gap_weeks`+ calendar weeks with NO load (ADR 0011) — bounded by absence because that is the only boundary in the data that is not a judgment call. Each block carries `start_date`/`end_date`, `weeks`, `total_load`, `mean_weekly_load`, `sessions`, `slope_tss_per_week` + `trend_r2` + `trend_known`, `easy_pct`/`moderate_pct`/`hard_pct` + `polarization_known`, and `ftp_family`/`ftp_lo`/`ftp_hi` + `ftp_known`. **Blocks are described, never named** — there is no base/build/peak field and you should not invent one from the numbers: load observes volume, not intent. A low `trend_r2` means the block had NO trend, which is an answer, not missing data; `trend_known: false` means under three weeks, where r2 would be 1 by construction. A long block is genuinely long — it is never subdivided, so a 47-week block means 47 weeks of unbroken training. `ftp_family` names WHOSE threshold the range is (a rowing threshold and a cycling FTP are different quantities). In-band errors: `no_activities` |
