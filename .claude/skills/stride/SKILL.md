...
- Junk HR (outside 35–220 bpm) is filtered at analyze time for stream SAMPLES, so sessions with bad straps
  ...
  is raw (unfiltered).
- The same 35–220 bound is applied again at REPORT time by `progress`'s EF and speed/HR lenses, by `activity`'s `baselines.ef`, by the load ladder's HR rung, and by `top hr` — four consumers, three different consequences. `progress` counts the refused session in `hidden_lens` and still shows the row; `activity` publishes `ef.known: false`; the ladder falls to its next rung and says so in `load_model`; **`top hr` does not rank it at all**, so a refused reading is absent from that list and its id is not recoverable from the JSON. If you need to find the broken sessions themselves, use `stride activities --json` and look for an `avg_hr` under 35 or over 220 — that is the one surface that publishes them raw. That one does not touch TSS — it decides whether a session can be scored and trended at all, and a session it refuses is counted in `hidden_lens` rather than shown.
