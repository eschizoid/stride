...
1. **The hard guard is closed-set equality.** Where a producer has finitely many outputs — verdict templates, band labels, state ids — every output is pinned by full-string equality in an `expect`. A reworded template fails the build. This is the guard that actually holds, and five producers carry it today: `form_state`, the compare verdict, `band_days_phrase`, `tte_screen`'s phrases, and `trend_label`. It is not yet every finite producer in the codebase; the ones without it are not guarded, whatever the sweep suggests.
...
- Adding a prose output surface means adding it to the boundary sweep in `src/Render.roc` — `compare_verdict`, `form_trend_phrase`, `band_days_phrase`, `warming_up_note`, `trend_label` and `tte_screen` are swept today. `season_screen` deliberately renders no verdict line at all, which is the cleanest way to satisfy this rule.
