...
1. **The hard guard is closed-set equality.** Where a producer has finitely many outputs — verdict templates, band labels, state ids — every output is pinned by full-string equality in an `expect`. A reworded template fails the build. This is the guard that actually holds, and five producers carry it today: `form_state`/`form_label`, the compare verdict, `band_days_phrase`, `trend_label`, and `backfill_note` (whose producer side is a TAG, so the compiler closes the set; three composed expects plus a List.all over the produced labels tie tag to string, one by equality and two by prefix). `tte_screen`'s phrases carry only the denylist sweep. It is not every finite producer in the codebase, and the ones without it are not guarded whatever the sweep suggests — the first draft of this very sentence credited `tte_screen` with an equality pin it does not have, which is the failure mode in miniature.
