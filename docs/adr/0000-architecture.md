...
policy), `Csv/Streams/Drain`. `Schema.roc` is pure DDL with no expects. `Output.roc` owns the envelope and is effectful.
...
the validator. Where a payload carries a closed record ANNOTATION — `sync`'s does, at
`Strava.roc`'s `payload :` binding — an added, removed or retyped key fails `roc check`.
That guards the payload against diverging from its own DECLARED SHAPE, and nothing more.
A command whose renderer is an unannotated inline closure has no such guard at all: the
compiler infers an OPEN record and a new key compiles clean. Even where the annotation
exists, widening it leaves the build green with the schema untouched
