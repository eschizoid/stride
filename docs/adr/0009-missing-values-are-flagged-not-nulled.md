...
- **Impossible-zero fields** (`np_w`, `avg_hr`, `intensity`): 0 means "not available", and the row carries `power_known` / `intensity_known` / `hr_known` booleans. `ftp_used` is impossible-zero too but ships NO flag, deliberately: analyze always *binds* it — never NULL, and 0 whenever the sport has no derivable FTP (strength sports, unstreamed activities). A flag decoded from stored NULLs would therefore be all-true and carry no information, which is not the same thing as the value always being present. Readers discriminate on `ftp_used > 0`, as `doctor` does for `derived_ftp_sports` and as the ladder does when it falls through to pace/HR/RPE. Four fields and three flags is not an oversight, and listing them together as though it were four-for-four is how that gets "fixed" by someone adding a phantom `ftp_used_known`. The flags decode the **stored NULLs** (`CASE WHEN … IS NULL` in the same SELECT), never the coalesced magnitudes — the database already holds the distinction losslessly, and magnitudes lie (np can be present while intensity is NULL: power stream, no FTP yet).
...
- Consumers (the coach skill first among them) read flags, never magnitudes. The contract lives as the comment block in `src/Output.roc` and the gotcha in `skills/stride/SKILL.md`; e2e pins both flag presence and flag discrimination (a power ride vs an HR-only row).
