    ...
    zone_sum! = |path, cutoff|
        Sqlite.query!({
            ...
            query:
                ...
                \\       COALESCE(SUM(CASE WHEN s.raw_json LIKE '%"heartrate"%' OR s.raw_json LIKE 'stride-stream-v1:h=1;%' THEN 1 ELSE 0 END),0) AS hr_streams,
                \\       COALESCE(SUM(CASE WHEN s.raw_json LIKE '%"heartrate"%' OR s.raw_json LIKE '%"watts"%' OR s.raw_json LIKE '%"distance"%' OR s.raw_json LIKE 'stride-stream-v1:h=1;%' OR s.raw_json LIKE 'stride-stream-v1:%w=1;%' OR s.raw_json LIKE 'stride-stream-v1:%d=1%' THEN 1 ELSE 0 END),0) AS intensity_streams
        ...
        match List.first(rows) {
            ...
            Ok(a) => {
                ...
                detail =
                    match raw_opt {
                        NotNull(raw) =>
                            if Str.starts_with(raw, "stride-stream-v1:") {
                                ActivityHelper.activity_detail!(path, aid, pi_ftp)?
                            } else {
