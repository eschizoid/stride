...
dispatch! = |cmd|
    match cmd {
        ...
        Command.Compare(period) => compare!(period)
        Command.Activities(c, sport) => activities!(c, sport)
        Command.Top(metric, c, sport) => top!(metric, c, sport)
        Command.Import(src) => import_archive!(src)
        Command.Rate(target, rpe_str) => rate!(target, rpe_str)
        Command.Progress(name) => progress!(name)
        Command.Activity(id_str) => activity!(id_str)
        Command.Load(days) => load_series!(days)
