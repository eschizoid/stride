...
Db :: [].{
	...
	load_prs! = |db|
		match Sqlite.query!({ db, query: "SELECT '5s' AS rung, 5 AS secs, CAST(COALESCE((SELECT ROUND(m.best_5s_w) FROM activities a JOIN activity_metrics m ON m.activity_id = a.id WHERE m.best_5s_w > 0 ORDER BY m.best_5s_w DESC, a.start_local ASC LIMIT 1), 0) AS INTEGER) AS w, CAST(COALESCE((SELECT substr(a.start_local, 1, 10) FROM activities a JOIN activity_metrics m ON m.activity_id = a.id WHERE m.best_5s_w > 0 ORDER BY m.best_5s_w DESC, a.start_local ASC LIMIT 1), '') AS TEXT) AS day UNION ALL SELECT '15s' AS rung, 15 AS secs, CAST(COALESCE((SELECT ROUND(m.best_15s_w) FROM activities a JOIN activity_metrics m ON m.activity_id = a.id WHERE m.best_15s_w > 0 ORDER BY m.best_15s_w DESC, a.start_local ASC LIMIT 1), 0) AS INTEGER) AS w, CAST(COALESCE((SELECT substr(a.start_local, 1, 10) FROM activities a JOIN activity_metrics m ON m.activity_id = a.id WHERE m.best_15s_w > 0 ORDER BY m.best_15s_w DESC, a.start_local ASC LIMIT 1), '') AS TEXT) AS day UNION ALL SELECT '30s' AS rung, 30 AS secs, CAST(COALESCE((SELECT ROUND(m.best_30s_w) FROM activities a JOIN activity_metrics m ON m.activity_id = a.id WHERE m.best_30s_w > 0 ORDER BY m.best_30s_w DESC, a.start_local ASC LIMIT 1), 0) AS INTEGER) AS w, CAST(COALESCE((SELECT substr(a.start_local, 1, 10) FROM activities a JOIN activity_metrics m ON m.activity_id = a.id WHERE m.best_30s_w > 0 ORDER BY m.best_30s_w DESC, a.start_local ASC LIMIT 1), '') AS TEXT) AS day UNION ALL SELECT '1min' AS rung, 60 AS secs, CAST(COALESCE((SELECT ROUND(m.best_60s_w) FROM activities a JOIN activity_metrics m ON m.activity_id = a.id WHERE m.best_60s_w > 0 ORDER BY m.best_60s_w DESC, a.start_local ASC LIMIT 1), 0) AS INTEGER) AS w, CAST(COALESCE((SELECT substr(a.start_local, 1, 10) FROM activities a JOIN activity_metrics m ON m.activity_id = a.id WHERE m.best_60s_w > 0 ORDER BY m.best_60s_w DESC, a.start_local ASC LIMIT 1), '') AS TEXT) AS day UNION ALL SELECT '5min' AS rung, 300 AS secs, CAST(COALESCE((SELECT ROUND(m.best_300s_w) FROM activities a JOIN activity_metrics m ON m.activity_id = a.id WHERE m.best_300s_w > 0 ORDER BY m.best_300s_w DESC, a.start_local ASC LIMIT 1), 0) AS INTEGER) AS w, CAST(COALESCE((SELECT substr(a.start_local, 1, 10) FROM activities a JOIN activity_metrics m ON m.activity_id = a.id WHERE m.best_300s_w > 0 ORDER BY m.best_300s_w DESC, a.start_local ASC LIMIT 1), '') AS TEXT) AS day UNION ALL SELECT '10min' AS rung, 600 AS secs, CAST(COALESCE((SELECT ROUND(m.best_600s_w) FROM activities a JOIN activity_metrics m ON m.activity_id = a.id WHERE m.best_600s_w > 0 ORDER BY m.best_600s_w DESC, a.start_local ASC LIMIT 1), 0) AS INTEGER) AS w, CAST(COALESCE((SELECT substr(a.start_local, 1, 10) FROM activities a JOIN activity_metrics m ON m.activity_id = a.id WHERE m.best_600s_w > 0 ORDER BY m.best_600s_w DESC, a.start_local ASC LIMIT 1), '') AS TEXT) AS day UNION ALL SELECT '20min' AS rung, 1200 AS secs, CAST(COALESCE((SELECT ROUND(m.best_20min_w) FROM activities a JOIN activity_metrics m ON m.activity_id = a.id WHERE m.best_20min_w > 0 ORDER BY m.best_20min_w DESC, a.start_local ASC LIMIT 1), 0) AS INTEGER) AS w, CAST(COALESCE((SELECT substr(a.start_local, 1, 10) FROM activities a JOIN activity_metrics m ON m.activity_id = a.id WHERE m.best_20min_w > 0 ORDER BY m.best_20min_w DESC, a.start_local ASC LIMIT 1), '') AS TEXT) AS day UNION ALL SELECT '60min' AS rung, 3600 AS secs, CAST(COALESCE((SELECT ROUND(m.best_3600s_w) FROM activities a JOIN activity_metrics m ON m.activity_id = a.id WHERE m.best_3600s_w > 0 ORDER BY m.best_3600s_w DESC, a.start_local ASC LIMIT 1), 0) AS INTEGER) AS w, CAST(COALESCE((SELECT substr(a.start_local, 1, 10) FROM activities a JOIN activity_metrics m ON m.activity_id = a.id WHERE m.best_3600s_w > 0 ORDER BY m.best_3600s_w DESC, a.start_local ASC LIMIT 1), '') AS TEXT) AS day", bindings: [] }) {
			Err(_) => []
			Ok(rows) =>
				List.keep_oks(rows, |r| {
					rg = r.str("rung") ? |_| "bad"
