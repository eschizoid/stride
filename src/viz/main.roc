...
update! = |model0, program_input| {
	...
	if d.key_pressed(KeyEscape) {
		...
		row_hit =
			if view == 3 and Mouse.button_pressed(d.mouse, Left) and m.x >= 36.0 and (if model.detail_day != "" (m.x < win.w - 540.0) else m.x < win.w - 40.0) and m.y >= 134.0 {
		...
		over_row = view2 == 3 and m.x >= 36.0 and (if detail_day2 != "" (m.x < win.w - 540.0) else m.x < win.w - 40.0) and m.y >= 134.0 and m.y < 134.0 + U64.to_f32(row_count) * 24.0
