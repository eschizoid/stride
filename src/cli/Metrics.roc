...
Metrics :: [].{
    ...
    # ── resampling ──────────────────────────────────────────────────────
    # The 1 Hz stream discipline lives in core.Series — one implementation
    # for both binaries (the window's pace channel resamples the same
    # streams; a second copy of a resampler is how one surface stitches
    # across a pause the other refuses). These bindings keep the Metrics.*
    # spelling every engine call site and expect uses; the measured
    # sort-vs-check numbers and the mode rationale live with the bodies.

    max_fill_gap : I64
    max_fill_gap = Series.max_fill_gap

    resample_1s_pairs : List({ t : I64, v : F64 }), [Hold, Interpolate] -> List({ t : I64, v : F64 })
    resample_1s_pairs = Series.resample_1s_pairs

    ascending_by_t : List({ t : I64, v : F64 }) -> Bool
    ascending_by_t = Series.ascending_by_t

    sorted_by_t : List({ t : I64, v : F64 }) -> List({ t : I64, v : F64 })
    sorted_by_t = Series.sorted_by_t

    resample_1s : List({ t : I64, v : F64 }) -> List(F64)
    resample_1s = Series.resample_1s

    resample_1s_linear : List({ t : I64, v : F64 }) -> List({ t : I64, v : F64 })
    resample_1s_linear = Series.resample_1s_linear
    ...
    # ── splits ──────────────────────────────────────────────────────────
    # The splits computation lives in core.Series — one implementation for
    # both binaries (the window draws the same rows beside a pace trace).
    # These bindings keep the Metrics.* spelling every engine call site and
    # expect uses; the boundary, tail-floor, dropout and gain rules live
    # with the body.
    Split : Series.Split
    ...
    expect {
        # 100 m every second: 300 m splits close at t=3,6,9 with no tail -
        # the last boundary lands exactly on the final sample
        dist = Iter.fold((0.U64..<10).iter(), [], |acc, i| List.append(acc, { t: (i).to_i64_wrap(), v: (i).to_f64() * 100.0 }))
        hr = Iter.fold((0.U64..<10).iter(), [], |acc, i| List.append(acc, { t: (i).to_i64_wrap(), v: 150.0 }))
        rows = splits(dist, [], hr, 300.0)
        List.len(rows) == 3
        and (match List.first(rows) {
            Ok(r) => r.elapsed_s == 3 and (r.distance_m - 300.0).abs() < 0.001 and r.hr_known and (r.avg_hr - 150.0).abs() < 0.001 and !r.elev_known
            Err(_) => Bool.False
        })
    }
    ...
    expect {
        ...
        List.len(rows) == 2
        and (match List.first(rows) {
            Ok(r) => r.elapsed_s == 21 and !r.hr_known
            Err(_) => Bool.False
        })
        and (match List.last(rows) {
            Ok(r) => (r.distance_m - 100.0).abs() < 0.5 and r.n == 2
            Err(_) => Bool.False
        })
    }

    expect {
        # climbing 2 m per second for the first 300 m only: the first split
        # carries the gain, the second is flat, and both know the altitude
        # stream exists
        dist = Iter.fold((0.U64..<7).iter(), [], |acc, i| List.append(acc, { t: (i).to_i64_wrap(), v: (i).to_f64() * 100.0 }))
        alt = Iter.fold((0.U64..<7).iter(), [], |acc, i| List.append(acc, { t: (i).to_i64_wrap(), v: if i <= 3 ((i).to_f64() * 2.0) else 6.0 }))
        rows = splits(dist, alt, [], 300.0)
        List.len(rows) == 2
        and (match List.first(rows) {
            Ok(r) => (r.elev_gain_m - 6.0).abs() < 0.001 and r.elev_known
            Err(_) => Bool.False
        })
        and (match List.last(rows) {
            Ok(r) => (r.elev_gain_m).abs() < 0.001 and r.elev_known
            Err(_) => Bool.False
        })
    }
