...
ReportSessions :: [].{
    ...
    activities! = |limit, sport_filter| {
        ...
        rows = Sqlite.query_many!({
            path: Path.utf8(path),
            query:
                \\SELECT a.id AS id, COALESCE(substr(a.start_local, 1, 10), '') AS date, ${Report.date_known_sql} AS date_known, ${Report.time_known_sql} AS time_known, a.sport_type AS sport, a.name AS name,
                \\       a.moving_time AS moving_time, CAST(COALESCE(a.distance,0) AS REAL) AS distance_m,
                \\       CAST(COALESCE(m.tss,0) AS REAL) AS tss, CAST(COALESCE(m.normalized_power,0) AS REAL) AS np_w,
                \\       CAST(COALESCE(m.intensity_factor,0) AS REAL) AS intensity,
                \\       COALESCE(m.z1_s,0) AS z1_s, COALESCE(m.z2_s,0) AS z2_s, COALESCE(m.z3_s,0) AS z3_s,
            ...
            rows: |cols| |stmt| {
                ...
                time_known = Sqlite.i64("time_known")(cols)(stmt)?
                sport = Sqlite.str("sport")(cols)(stmt)?
                name = Sqlite.str("name")(cols)(stmt)?
                moving_time = Sqlite.i64("moving_time")(cols)(stmt)?
                distance_m = Sqlite.f64("distance_m")(cols)(stmt)?
                ...
                zones_known = Sqlite.i64("zones_known")(cols)(stmt)?
                load_model = Sqlite.str("load_model")(cols)(stmt)?
                Ok({ id, date, date_known: date_known != 0, time_known: time_known != 0, sport, name, moving_time, distance_m, tss, load_model, np_w, power_known: power_known != 0, intensity, intensity_known: intensity_known != 0, z1_s, z2_s, z3_s, z4_s, z5_s, zones_known: zones_known != 0, hard_s, relative_effort, avg_hr, hr_known: hr_known != 0 })
            },
        })?
        if Output.json_mode!({})
            Output.emit_ok!(rows)
            ...
            Ok({ col, header }) => {
                ...
                rows = Sqlite.query_many!({
                    ...
                    rows: |cols| |stmt| {
                        ...
                        intensity_known = Sqlite.i64("intensity_known")(cols)(stmt)?
                        hr_known = Sqlite.i64("hr_known")(cols)(stmt)?
                        Ok({ id, date, date_known: date_known != 0, time_known: time_known != 0, sport, name, moving_time, distance_m, tss, np_w, power_known: power_known != 0, intensity, intensity_known: intensity_known != 0, avg_hr, hr_known: hr_known != 0, output_kj })
                    },
                })?
                if Output.json_mode!({})
                    Output.emit_ok!(rows)
