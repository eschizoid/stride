...
- **`just skill-shapes` holds SKILL.md's payload literals to the schemas.** `command-claims` resolves command NAMES; this resolves payload KEYS, which nothing did — a field could be added to a schema, marked required, shipped, and never reach the coaching agent's instruction sheet. That happened twice in three days in consecutive commits of one PR (`groups[].hidden`, then `hidden_lens`/`hidden_scope`), both caught by a human reading the diff. The failure is silent in the direction that matters: an agent given an undocumented field does not error, it simply never uses it. Two directions, joined on the schema column of `src/Command.roc`'s table so the pairing is not restated anywhere. Schema to doc fires only for objects SKILL.md ENUMERATES — coverage at or above 70% of an object's properties — because SKILL.md summarises most payloads on purpose and demanding full coverage everywhere reports 166 mismatches on a healthy tree. The two populations separate cleanly with nothing between them: full listings sit at 100%, deliberate summaries at 50% and below. Doc to schema fires on any key inside a `{...}` literal that no schema property backs, which catches documentation still promising a renamed field. Presence is tested against the whole ROW rather than the literal, because a field explained in prose is documented even when it sits outside the braces.
