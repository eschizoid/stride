...
version = "stride 0.1.1" # x-release-please-version
...
prescriptions! = |{}|
    ...
    if json_mode!({}) then
        ...
        Stdout.line!(Render.render_table(
            ["date", "type", "status", "detail", "id"],
            List.map(ordered, |p| [p.target_date, p.session_type, p.status, p.detail, Num.to_str(p.id)]),
        ))
